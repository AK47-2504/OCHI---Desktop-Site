const Footer = () => {
  ...
  const menuItems = ["Home", "About", "Our Work", "Insights", "Contact Us"];

  return (
    <div data-scroll data-scroll-section data-scroll-speed='1' className="w-full gap-5 h-screen bg-gray-200 p-20 flex font-['Neue Montreal']">
      {/* Left Section */}
      <div className="flex flex-col justify-between h-full w-1/2 font-['TradeGothic_LT_CondEighteen']">
        <div className="f-heading">
          <h1 className="text-[8vw] font-semibold uppercase -mb-3 leading-none -tracking-[.7vw]">
            Eye-
          </h1>
          <h1 className="text-[8vw] font-semibold uppercase leading-none -tracking-[.7vw]">
            Opening
          </h1>
        </div>
        <svg
      ...
      {/* Right Section */}
      <div className="w-1/2">
        <h1 className="text-[8vw] font-semibold uppercase -mb-3 leading-none -tracking-[.7vw] font-['TradeGothic_LT_CondEighteen']">
          Presentations
        </h1>
        <div className="w-full justify-between items-center mt-[7vw] details flex">
          {/* Social Links */}
          <div className="details-left mb-10">
            <div className="mb-5 text-left">
              <h4 className="mb-3">S:</h4>
              {socialLinks.map((link) => (
                <a key={link} className="flex underline" href="#">
                  {link}
                </a>
              ))}
            </div>
            {/* Locations */}
            <div className="mb-5 text-left">
              <h4 className="mb-3">L:</h4>
              {locations.map((location) => (
                <p key={location} className="underline">
                  {location}
                </p>
              ))}
            </div>
          ...
          {/* Menu Items */}
          <div className="details-right px-[10vw]">
            <div>
              <h4 className="mb-3">M:</h4>
              {menuItems.map((item) => (
                <a key={item} className="flex underline" href="#">
                  {item}
                </a>
              ))}
            </div>
